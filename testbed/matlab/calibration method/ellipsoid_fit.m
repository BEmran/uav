...
if f==0  % any axes (rotated ellipsoid)
    D = [x.*x,  y.*y,  z.*z,  2*x.*y,2*x.*z,2 *y.*z, 2*x,2*y,2*z];
elseif f == 1% XYZ axes (non-rotated ellipsoid)
    D=[x.*x,  y.*y,  z.*z,                        2*x,2*y,2*z];
elseif f == 2 %   and radius x=y
    
    D=[x.*x+y.*y,    z.*z,                        2*x,2*y,2*z];
elseif f == 3%   and radius x=z
    
    D=[x.*x+z.*z,    y.*y,                        2*x,2*y,2*z];
elseif f == 4%   and radius y=z
    
    D=[y.*y+z.*z,    x.*x,                        2*x,2*y,2*z];
elseif f == 5 %   and radius x=y=z (sphere)
    
    D=[x.*x+y.*y+z.*z,                            2*x,2*y,2*z];
end
%v = (D'*D) .\ (D'*ones(length(x),1)); % least square fitting
v = (D'*D)^-1 * D' * ones(length(x),1);
if f == 0 % rotated ellipsoid
    ...
    elseif f == 4
        v = [ v(2) v(1) v(1) 0
            0 0 v(3) v(4) v(5) ];
    elseif f == 5
        v = [ v(1) v(1) v(1) 0 0 0 v(2) v(3) v(4) ]; % sphere
    end
